...
const PLACES = {
  "書桌": ["滑鼠線", "螢幕底座", "杯底水痕"], "房門": ["鑰匙孔", "門鍊", "窺視孔"],
  "牆邊": ["牆紙裂縫", "鬆動插座", "床頭板"], "走廊": ["監視器鏡頭", "感應燈", "消防箱玻璃"],
  "樓梯間": ["防火門玻璃", "扶手灰塵", "緊急照明燈"], "電梯廳": ["樓層按鈕", "電梯門縫", "緊急通話孔"],
  "洗衣間": ["洗衣機圓窗", "排水孔", "投幣口"], "維修夾層": ["溫熱水管", "電線束", "檢修孔"],
  "一樓大廳": ["住戶信箱", "玻璃大門", "警衛桌電話"], "屋頂": ["水塔鐵梯", "避雷線", "生鏽欄杆"],
  "騎樓": ["店面玻璃", "閃爍招牌", "路口反光鏡"]
};
...
const MOVE_RESULTS = [
  (target) => `假呼吸把它引向原位。你滑進${target}時，身後的手機忽然用你的聲音喊停。`,
...
const CONTACT_RESULTS = [
  ...
  (target, moves) => moves ? `數到七時，${target}的門先開了。另一個你走進去，卻把影子交給你。` : "你假裝做出選擇，黑暗裡果然伸出一隻手。"
];
...
const first = (text, max = 54) => clip(sentences(text)[0] || text, max);
const last = (text, max = 30) => { const parts = sentences(text); return clip(parts[parts.length - 1] || text, max); };
const quote = (text) => clip(String(text || "").match(/[「『“"]([^」』”"]+)[」』”"]/u)?.[1] || last(text).replace(/[。！？]$/, ""), 30);
function original(element) { if (!element) return ""; if (!element.dataset.narrativeOriginal) element.dataset.narrativeOriginal = element.textContent || ""; return element.dataset.narrativeOriginal; }
function setText(element, value) { if (!element) return; original(element); if (element.textContent !== value) element.textContent = value; }
function identity() {
...
const destination = (label, fallback) => String(label || "").match(/(?:前往|走到)(.+)$/)?.[1]?.trim() || fallback;

export default function NarrativeRefreshV3({ children }) {
  ...
  useEffect(() => {
    let syncing = false;
    const restore = () => document.querySelectorAll("[data-narrative-hidden]").forEach((element) => {
    ...
    const sync = () => {
      ...
      if (paragraphs.length >= 3) {
        setText(paragraphs[0], `${pick(ARRIVALS, seed, 9)} ${first(original(paragraphs[1]), 48)}`);
        setText(paragraphs[1], `黑暗裡有人說：「${quote(original(paragraphs[2]))}」`);
      ...
      if (result && Number.isInteger(selectedRef.current)) {
        ...
        const text = selectedRef.current === 0 ? pick(SETUP_RESULTS, meta.seed, 6)(meta.item)
          : selectedRef.current === 1 ? pick(MOVE_RESULTS, meta.seed, 7)(meta.moveTarget)
    ...
    sync();
    return () => { observer.disconnect(); document.removeEventListener("click", capture, true); restore(); };
  }, []);
